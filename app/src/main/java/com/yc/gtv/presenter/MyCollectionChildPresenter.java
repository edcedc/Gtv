...
public class MyCollectionChildPresenter extends MyCollectionChildContract.Presenter{
    ...
    @Override
    public void onRequest(int type, int pagerNumber) {
        String url;
        if (type == Constants.COLLECTION_VIDEO){
            url = CloudApi.collectPageQueryVideoCollect;
        }else {
            url = CloudApi.collectPageQueryGalleryCollect;
        }
    ...
    public void onDel(List<String> listBean) {
        CloudApi.collectDelete(listBean)
                .doOnSubscribe(new Consumer<Disposable>() {
                    @Override
                    public void accept(Disposable disposable) throws Exception {
                        mView.showLoading();
