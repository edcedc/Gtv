...
public class DateUtils {
     ...
     *  减几天
     */
    public static String reduceTime(String time, int dayNumber){
        if (StringUtils.isEmpty(time))return "";
        SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            Date date = sf.parse(time); // 指定日期
            Date newDate = addDate(date, -dayNumber);
            return sf.format(newDate);
    ...
    //本项目需求获取当前时间零点
    public static String zeroTime(String time){
